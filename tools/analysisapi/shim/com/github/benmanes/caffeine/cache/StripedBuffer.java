/*
 * AIDE-OS shim. Adapted from Caffeine 2.9.3, Apache License 2.0, by replacing
 * three members that reach a JDK-internal field Android does not have. The
 * original header and the rest of the file are unchanged; see
 * tools/analysisapi/FINDINGS.md for why this class needs replacing at all.
 *
 * Upstream: https://github.com/ben-manes/caffeine
 */
/*
...
abstract class StripedBuffer<E> implements Buffer<E> {
  ...
  /**
   * AIDE-OS: the probe, per thread, without `Thread.threadLocalRandomProbe`.
   *
   * Upstream reads that field directly through `Unsafe`, duplicating what
   * `ThreadLocalRandom` does internally "because of packaging restrictions".
   * **Android's `Thread` does not declare it**, so the original
   * `objectFieldOffset` call throws `NoSuchFieldException` out of this class's
   * static initialiser -- taking every Caffeine cache with it, and arriving at
   * the caller as an `IllegalStateException` whose message is only a generated
   * class name.
   *
   * A `ThreadLocal` gives the same thing the field gave: a per-thread `int`
   * that this class seeds once and then advances itself. It is strictly slower
   * than a field read at a known offset, and that cost buys running at all.
   */
  static final ThreadLocal<int[]> PROBE = new ThreadLocal<>();
  ...
  // AIDE-OS: upstream annotates this `Buffer<E> @Nullable[]`. The annotation
  // is checker-framework's, documentation-only at runtime, and keeping it would
  // mean pinning a compile-only artifact that never ships. Dropped rather than
  // depended on; nothing about the field's behaviour changes.
  ...
  final void expandOrRetry(E e, boolean wasUncontended) {
    int h;
    if ((h = getProbe()) == 0) {
      // AIDE-OS: upstream calls ThreadLocalRandom.current() purely for its
      // side effect of seeding Thread.threadLocalRandomProbe. With the field
      // gone that does nothing, and h would stay 0 forever -- every thread
      // hashing to stripe 0, which is silent, correct, and unstriped. Seed the
      // ThreadLocal directly, avoiding 0 because 0 is this code's "unseeded".
      int seed = ThreadLocalRandom.current().nextInt();
      int[] cell = new int[] { (seed == 0) ? 1 : seed };
      PROBE.set(cell);
      h = cell[0];
      wasUncontended = true;
    }

...
| ✅ | Spike R11 — the Gradle path's foundation | The rootfs route is **closed** (PRoot works; a musl guest cannot be executed) and **unnecessary**. Termux's OpenJDK 21 is a Bionic binary, and with a launcher of our own calling `JNI_CreateJavaVM` it runs **in the app's own process**: `javac` compiles a class in ~400 ms and the VM runs it. Forking needs `-Djdk.lang.Process.launchMechanism=vfork`. `tools/rootfs/FINDINGS.md` |

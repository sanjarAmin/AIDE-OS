...
*As of 2026-09-15. **Every milestone M0–M11 is met.** M12, the debugger, is
...
| 🟢 | **M9** Gradle path | `:engine:gradle` builds an Android project on the device with the project's own Gradle, on Termux's OpenJDK started by our launcher — accepted by the platform's package parser, and a **two-module** project builds too. Verified on x86_64 and on Android 16 arm64, which took disabling heap pointer tagging the emulator could never have shown. **Nothing is staged by hand any more**: the JDK, Gradle and build-tools install themselves, and the SDK root is composed from them. **The editor no longer waits for a build**: opening a Gradle project with nothing recorded *syncs* it — dependencies, `R`, `BuildConfig` and view binding resolve in about 20 s with nothing compiled — and Stop now ends the Gradle processes rather than only the panel's view of them. `engine/gradle/FINDINGS.md` §12, `tools/rootfs/FINDINGS.md` |
...
| **M12** Debugger | `:debugger`, a JDWP client, plus the agent the build puts inside a debug APK. **Not the shape this document assumed** — spike R15 found an app cannot reach another app's JDWP and does not have to: ART ships OpenJDK's `libjdwp.so`, and a debuggable process can attach it to itself | Breakpoint an app AIDE-OS built, stop it, read a named local, step a line, resume — **met, in the app**, 2026-09-13: a breakpoint set by tapping line 12's number, Debug, and the app stopped *in `onCreate`* (the build holds startup until the breakpoints are placed), came back in front of the IDE by itself, showed `this` and `savedInstanceState`, stepped to line 13 where `text = TextView` appeared, resumed and detached with the app still running. **What it took that nobody plans for: Android 14 freezes whichever of the two apps is off screen**, so each holds the other awake — `debugger/FINDINGS.md` §9. **Kotlin and the Gradle engine, 2026-09-14**: a Kotlin app stopped inside an inlined `sumOf` lambda and stepped round its loop, and a Gradle project whose debug variant installs as `….debug` stopped in `onCreate` -- the agent reaching it through an init script, the project untouched. Both drives found defects outside the debugger: every Kotlin build with a downloaded compiler killed the build process, a Kotlin app with no dependencies died on its first line for want of the stdlib, and the Gradle engine's JDK, Gradle and build tools were never offered for download and not found once installed — `debugger/FINDINGS.md` §11–14, `toolchain/manager/FINDINGS.md` §10. Not yet: persisted breakpoints, breakpoints that follow edits, a breakpoint inside a project's own inline function |
...
**The second session, 2026-09-08 and 09**, found nine more. Two went unexplained
for a day; one of them is now solved, by bisecting rather than reasoning about
it. The three share a shape with the first session's: two were KDocs

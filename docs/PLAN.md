...
| **M10** JS / C# | ~~QuickJS +~~ Node from Termux (R13) and **mono** rather than the .NET SDK, which Termux does not publish (R14) | Node project runs; C# console app compiles — **both halves proven on device**, so what remains is the toolchain component and the build wiring, not the risk |

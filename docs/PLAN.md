...
| 🟡 | Spike R12 — Kotlin intelligence | **The toolchain exists and loads on ART.** The Analysis API is not in what we ship and not on Maven Central, so it is fetched from JetBrains' own repository, **relocated onto the compiler's shaded namespace** (8877 references; the two are otherwise incompatible) and dexed to a 1.9 MB archive. On device it loads and its references resolve — 643 types across `KaSession`'s methods, none unshaded. **A session does not build yet** — the API's plugin descriptors will not resolve through the trimmed `intellij-core` the compiler bundles — and **that fails identically on a desktop JVM**, so what remains is aligning the IntelliJ platform rather than anything about Android. Latency against M3's 200 ms is still unmeasured. `tools/analysisapi/FINDINGS.md`, `tools/kotlinls/FINDINGS.md` |

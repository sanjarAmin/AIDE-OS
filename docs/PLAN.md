...
| 🟡 | Spike R14 — C# on the device | **Blocked, with the cause named.** Termux publishes no `dotnet` and Microsoft's builds are glibc, so the candidate is `mono` 6.14.1 — 222 MB, not the 9 MB the base package suggests. The runtime starts in **23 ms** and the compiler assembly reports its version, so the JIT and class library work. Then nothing can read a file: `mcs` answers `CS2001 ... could not be found` for a file that exists, **including `mcs.exe` itself**, which the runtime had just loaded. The REPL gives the real error — `System.IO` P/Invokes `libmono-native.so` by an **absolute path baked in at build time** under Termux's prefix, so `Interop.Sys`'s initializer throws and every file operation fails. A global `dllmap` did not lift it. Four candidate routes are listed, the cleanest being a mono built with a relocatable prefix. `tools/mono/FINDINGS.md` |
...
| **M10** JS / C# | ~~QuickJS +~~ Node from Termux, de-risked by spike R13 — runtime, npm and spawning all work; C# **blocked** at R14, and the .NET SDK is not available by this route at all | Node project runs; C# console app compiles — **the JS half is reachable, the C# half needs mono relocated first** |

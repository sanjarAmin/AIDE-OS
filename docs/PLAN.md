...
| 🟡 | Spike R12 — Kotlin intelligence | **Surveyed, not resolved.** The editor answers Java and C/C++ and returns nothing for a `.kt` file. The front end we already ship has FIR (4259 classes) but **no Analysis API** (0), and the standalone Analysis API is published to JetBrains' own repository rather than Maven Central — so completion is a second hosted toolchain, not a module. `tools/kotlinls/FINDINGS.md` |

...
| 🟡 | Spike R12 — Kotlin intelligence | **The toolchain exists and loads on ART.** The Analysis API is not in what we ship and not on Maven Central, so it is fetched from JetBrains' own repository, **relocated onto the compiler's shaded namespace** (8877 references; the two are otherwise incompatible) and dexed to a 1.9 MB archive. On device it loads and its references resolve — 643 types across `KaSession`'s methods, none unshaded. **It answers on a desktop JVM**: a standalone session resolves `String` to `kotlin/String` and `List<Int>` to `kotlin/collections/List<kotlin/Int>` — resolution, not parsing. Getting there took flattening the plugin descriptors, adding kotlinx-serialization, and finding that kotlinx-collections-immutable was never a missing dependency but a **missed relocation**. On ART it stops in **Caffeine**, whose 3.x needs `System.getLogger` and whose 2.x needs `Thread.threadLocalRandomProbe` — neither of which Android has. Shimmable the way the compiler's four classes already are. Latency against M3's 200 ms is still unmeasured. `tools/analysisapi/FINDINGS.md`, `tools/kotlinls/FINDINGS.md` |
